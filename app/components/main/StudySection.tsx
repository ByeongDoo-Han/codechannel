...
// Define types for study and user selections
interface Study {
  studyId: number;
  studyName: string;
  createdBy: string;
...
interface StudySectionProps {
  openStudyDetailPopup: (studyId: string) => void;
...
//   const response = await axios.get('http://localhost:8080/api/v1/studies');
//   return response.data;
// }

export default function StudySection({openStudyDetailPopup, handleAttendance, userSelections, isDarkMode }: StudySectionProps) {
      ...
      <div className="space-y-3 sm:space-y-4">
        {studies.map((study: Study) => (
          <div 
            ...
            onClick={() => openStudyDetailPopup(study.studyId.toString())}
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`text-xs sm:text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {study.studyDate} • {study.participants.length}명 참여
                  </p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                ...
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    handleAttendance(study.studyId.toString(), 'attend');
                  }}
                  className={`px-4 py-2 text-xs sm:px-3 sm:py-1 rounded-full border transition-colors ${
                    userSelections[study.studyId.toString()] === 'attend'
                      ? (isDarkMode 
                ...
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    handleAttendance(study.studyId.toString(), 'unattend');
                  }}
                  className={`px-4 py-2 text-xs sm:px-3 sm:py-1 rounded-full border transition-colors ${
                    userSelections[study.studyId.toString()] === 'unattend'
                      ? (isDarkMode 
